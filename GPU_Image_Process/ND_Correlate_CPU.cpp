...
extern "C" __declspec ( dllexport ) void _stdcall ND_Correlate_CPU(
	const double* Y, 
	const double* X, 
	const int Y_length,
	const int Dims, 
...
//# pragma omp parallel for
	for (int i = 0; i < Y_length; i++)
	{
		...
		for (int iDim = 0; iDim < Dims; iDim++)
		{
			...
			int LocalIndex = (int) floor((xView(i, iDim) - rangeView(iDim, 0)) / (rangeView(iDim, 1) - rangeView(iDim, 0)) * Nbins);
...
//# pragma omp parallel for	// now we can calculate std
	for (int i = 0; i < Y_length; i++)
	{
		int idx = Index[i];
		//omp_set_lock(&BinLocks[idx]); // put the lock on
		Std[idx] = Std[idx] + (Avg[idx] - yView(i)) * (Avg[idx] - yView(i)); // add up the variance
		//omp_unset_lock(&BinLocks[idx]); // turn the lock off
	}
...
//# pragma omp parallel for
	for (int iBin = 0; iBin < NumLocks; iBin++)
	{
		if (Count[iBin] > 0)
		{
			Std[iBin] = sqrt(Std[iBin] / Count[iBin]); // now square root the variance divided by the bincount to get the Std
